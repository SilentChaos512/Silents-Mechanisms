...
public class MixerScreen extends AbstractMachineBaseScreen<MixerContainer> {
    ...
    protected void func_230459_a_(MatrixStack matrixStack, int mouseX, int mouseY) {
        if (isPointInRegion(28, 17, 13, 51, mouseX, mouseY)) {
            renderTooltip(matrixStack, TextUtil.fluidWithMax(container.getFluidInTank(0), MixerTileEntity.TANK_CAPACITY), mouseX, mouseY);
        }
        if (isPointInRegion(44, 17, 13, 51, mouseX, mouseY)) {
            renderTooltip(matrixStack, TextUtil.fluidWithMax(container.getFluidInTank(1), MixerTileEntity.TANK_CAPACITY), mouseX, mouseY);
        }
        if (isPointInRegion(60, 17, 13, 51, mouseX, mouseY)) {
            renderTooltip(matrixStack, TextUtil.fluidWithMax(container.getFluidInTank(2), MixerTileEntity.TANK_CAPACITY), mouseX, mouseY);
        }
        if (isPointInRegion(76, 17, 13, 51, mouseX, mouseY)) {
            renderTooltip(matrixStack, TextUtil.fluidWithMax(container.getFluidInTank(3), MixerTileEntity.TANK_CAPACITY), mouseX, mouseY);
        }
        if (isPointInRegion(116, 17, 13, 51, mouseX, mouseY)) {
            renderTooltip(matrixStack, TextUtil.fluidWithMax(container.getFluidInTank(4), MixerTileEntity.TANK_CAPACITY), mouseX, mouseY);
        }
